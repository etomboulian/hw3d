...
Melon::Melon(Graphics& gfx, std::mt19937& rng, std::uniform_real_distribution<float>& adist, 
    ...
    std::uniform_real_distribution<float>& rdist, std::uniform_int_distribution<int>& longdist, std::uniform_int_distribution<int>& latdist)
    ...
    chi(adist(rng)), theta(adist(rng)), phi(adist(rng))
{
    ...
    // deform verticies of model by linear tranform
    model.Transform(dx::XMMatrixScaling(1.0f, 1.0f, 1.0f));

    AddBind(std::make_unique<VertexBuffer>(gfx, model.verticies));
    AddIndexBuffer(std::make_unique<IndexBuffer>(gfx, model.indicies));
